/**
 * zNix board — SVG behavior card for a zNix manifest.
 *
 * Plug-and-play eyes on the box: render any manifest as a portable SVG —
 * address header, composition lanes (agents/services/schedules/pipelines),
 * the pipeline route as squares + arrows, the toolset as bounded-verb chips,
 * and the Inbox as the escalation counter. Same visual language as the ATLAS
 * GUI behavior board (BACKEND/logic/gui_behavior/spec.py card_svg).
 *
 * Zero dependencies: returns an SVG string — drop it into an <img src>,
 * an <object>, or a container's innerHTML.
 */
...
/**
 * One-card render of a zNix manifest.
 * @param {object} z the manifest (or a ZNix instance)
 * @param {{accent?: string, ok?: boolean, inbox?: number}} opts
 * @returns {string} standalone SVG
 */
export function boardCardSVG(z, opts = {}) {
  const W = 620;
  const H = 420;
  ...
  out.push(`<text x="${W - 24}" y="46" font-size="10" text-anchor="end" fill="#8f8fb5" font-family="monospace">${(z.modes || []).join(' · ')}</text>`);

  // ── Composition lanes ────────────────────────────────────────────────────
  let y = 84;
  for (const kind of kinds) {
    const items = z.composition?.[kind] || [];
    if (!items.length) continue;
    out.push(`<text x="24" y="${y}" font-size="10" fill="#6f738f" font-family="monospace" text-transform="uppercase">${esc(kind)}</text>`);
    let ix = 150;
    for (const it of items) {
      const label = it.name || it.role || String(it);
  ...
  const pipeline = (z.composition?.pipelines || [])[0];
